...
class PeopleRow extends StatefulWidget {
  /// [compact] = true renders up to 5 evenly spaced round avatars
  /// (used inside the blue balance header). Default is the original
  /// "People" grid with the More button.
  const PeopleRow({super.key, this.compact = false});

  final bool compact;

  static const double compactSize = 47;
  static const double compactGap = 11;
  static const int compactMax = 5;
...
class PeopleRowState extends State<PeopleRow> {
  ...
  Future<void> _openContact(_Person p) async {
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => ContactHistoryScreen(
  ...
  // ── Compact mode: evenly spaced round avatars for the blue header ──
  Widget _buildCompact() {
    const size = PeopleRow.compactSize;
    const gap = PeopleRow.compactGap;

    if (_loading || _people.isEmpty) {
      return const SizedBox(height: size);
    }

    final shown = _people.take(PeopleRow.compactMax).toList();
      ...
      children: [
        for (int i = 0; i < shown.length; i++) ...[
          ...
          _CompactAvatar(
            size: size,
            name: shown[i].name,
            initials: _initials(shown[i].name),
            color: _colorFor(shown[i].id),
            photoUrl: shown[i].photoUrl,
            onTap: () => _openContact(shown[i]),
          ),
        ],
      ],
    );
...
/// Round avatar with a soft translucent ring — matches the design.
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              color: photoUrl != null ? Colors.transparent : color,
              image: photoUrl != null
                  ? DecorationImage(
                image: NetworkImage(photoUrl!),
                fit: BoxFit.cover,
